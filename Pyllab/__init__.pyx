...
cdef class duelingCategoricalDQN:
    ...
    @ bint mod:= is a flag, if true we must read from a .bin if filename is != None, if it is true and filename == None we just set some parameters and we don't have any model (is used by copy_model)
     ...
     we are building the model from each class defined in python, we pass all the classes defined in python and et voilà we build in C the struct model
    '''
    def __cinit__(self,filename=None, dict d = None,bint mode = False, qr_dqn = False, Model shared=None, Model v_hid=None, Model v_lin=None, Model a_hid = None, Model a_lin = None,int input_size = 0, int action_size = 0, int n_atoms = 0, float v_min = 0, float v_max = 0, bint does_have_learning_parameters = True, bint does_have_arrays = True, bint is_only_for_feedforward = False):
        check_int(input_size)
        ...
        if filename != None:
            ...
            if self._is_only_for_feedforward:
                self.make_it_only_for_ff()
        if qr_dqn:
            Pyllab.set_is_qr(self._dqn, 1)
        self.is_qr_dqn = qr_dqn
    ...
    def save_from_number(self,int number_of_file):
        if self._dqn is NULL:
            return
        check_int(number_of_file)
        if self._does_have_arrays and self._does_have_learning_parameters:
            Pyllab.save_dueling_categorical_dqn(self._dqn, number_of_file)
    ...
    def compute_probability_distribution(self, inputs , int input_size):
        ...
        if self._does_have_arrays and self._does_have_learning_parameters:
            ...
            i = vector_is_valid(inputs)
            if not self.is_qr_dqn:
                Pyllab.compute_probability_distribution(<float*>&i[0], input_size, self._dqn)
    
            else:
                Pyllab.compute_probability_distribution_qr_dqn(<float*>&i[0], input_size, self._dqn)
    ...
    def compute_q_function(self, inputs, int input_size):
        ...
        if self._does_have_arrays and self._does_have_learning_parameters:
            if not self.is_qr_dqn:
                ...
                return from_float_to_ndarray(Pyllab.compute_q_functions_qr_dqn(self._dqn),self.action_size)
        return None
    ...
    def reset_weights_according_to_percentage_scores(self, float percentage):
        if self._dqn is NULL:
            return
        if percentage >= 1 or percentage <= 0:
            return
        Pyllab.reinitialize_weights_according_to_scores_dueling_categorical_dqn_only_percentage(self._dqn, percentage)
...
def py_copy_dueling_categorical_dqn(duelingCategoricalDQN dqn):
    ...
    if dqn._does_have_learning_parameters and dqn._does_have_arrays and not dqn._is_only_for_feedforward:
        if not dqn._is_from_char:
            ...
            return duelingCategoricalDQN(qr_dqn = dqn.is_qr_dqn, shared = shared, v_hid = v_hid, v_lin = v_lin , a_hid = a_hid, a_lin = a_lin, input_size = dqn.input_size,action_size = dqn.action_size,n_atoms = dqn.n_atoms,v_min = dqn.v_min,v_max = dqn.v_max)
        copy = Pyllab.copy_dueling_categorical_dqn(dqn._dqn)
        mod = duelingCategoricalDQN(qr_dqn = dqn.is_qr_dqn, mode = True, input_size = dqn.input_size, action_size = dqn.action_size, v_min = dqn.v_min, v_max = dqn.v_max, n_atoms = dqn.n_atoms)
        mod._dqn = copy
        return mod
...
def py_copy_dueling_categorical_dqn_without_learning_parameters(duelingCategoricalDQN dqn):
    ...
    if dqn._does_have_learning_parameters and dqn._does_have_arrays and not dqn._is_only_for_feedforward:
        if not dqn._is_from_char:
            ...
            return duelingCategoricalDQN(qr_dqn = dqn.is_qr_dqn, shared = shared, v_hid = v_hid, v_lin = v_lin , a_hid = a_hid, a_lin = a_lin, input_size = dqn.input_size,action_size = dqn.action_size,n_atoms = dqn.n_atoms,v_min = dqn.v_min,v_max = dqn.v_max,does_have_learning_parameters = False)
        copy = Pyllab.copy_dueling_categorical_dqn_without_learning_parameters(dqn._dqn)
        mod = duelingCategoricalDQN(qr_dqn = dqn.is_qr_dqn, mod = True,does_have_learning_parameters = False, input_size = dqn.input_size, action_size = dqn.action_size, v_min = dqn.v_min, v_max = dqn.v_max, n_atoms = dqn.n_atoms)
        mod._dqn = copy
        return mod
...
cdef class Rainbow:
    ...
    def __cinit__(self,duelingCategoricalDQN online_net, duelingCategoricalDQN target_net, float beta_priorization_increase = 0.05, float max_epsilon = 1, float min_epsilon = 0.1,
                  float diversity_driven_decay = 0, float diversity_driven_minimum = 0.001, float diversity_driven_maximum = 1, float epsilon_decay = 0.005,float epsilon = 0.99, float alpha_priorization = 0.4,
                  ...
                  float k_percentage = 1, float clipping_gradient_value = 0.2, float adaptive_clipping_gradient_value = 0.01, float diversity_driven_threshold = 0.05, float lr = 0.001, float lr_minimum = 0.0001, float lr_maximum = 0.1,
                  ...
                  int batch_size = 128,int threads = 128, int gd_flag = PY_ADAM, int max_buffer_size = 50000, int n_step_rewards = 3, int stop_epsilon_greedy = 500000, int epochs_to_copy_target = 1,
...
cdef class duelingCategoricalDQNBuffers:
    cdef size
    cdef float** b1
    cdef int** b2
    cdef duelingCategoricalDQN dqn
    
    def __cinit__(self,int size, duelingCategoricalDQN dqn):
        check_int(size)
        
        self.size = size
        self.dqn = dqn
        self.b1 = <float**>malloc(sizeof(float*)*size)
        self.b2 = <int**>malloc(sizeof(int*)*size)
        
        for i in range(size):
            self.b1[i] = <float*>malloc(sizeof(float)*Pyllab.get_array_size_scores_dueling_categorical_dqn(self.dqn._dqn))
            self.b2[i] = <int*>malloc(sizeof(int)*Pyllab.get_array_size_scores_dueling_categorical_dqn(self.dqn._dqn))
            for j in range((Pyllab.get_array_size_scores_dueling_categorical_dqn(self.dqn._dqn))):
                self.b2[i][j] = j
        
    def __dealloc__(self):
        Pyllab.free_matrix(<void**>self.b1,self.size)
        Pyllab.free_matrix(<void**>self.b2,self.size)
    
    
    def copy_indices(self, list l):
        if len(l) != self.size:
            return
        cdef duelingCategoricalDQN dqn
        for i in range(self.size):
            dqn = l[i]
            Pyllab.memcopy_indices_to_vector_dueling_categorical_dqn(dqn._dqn, self.b2[i])
    
    def copy_scores(self, list l):
        if len(l) != self.size:
            return
        cdef duelingCategoricalDQN dqn
        for i in range(self.size):
            dqn = l[i]
            Pyllab.memcopy_scores_to_vector_dueling_categorical_dqn(dqn._dqn, self.b1[i])
    
    def redefine_scores(self, list l, float step, by_indices = True):
        if len(l) != self.size:
            return
        cdef duelingCategoricalDQN dqn = l[0]
        if by_indices:
            Pyllab.scores_and_indices_readapter(self.b2, self.b1, len(l), Pyllab.get_array_size_scores_dueling_categorical_dqn(dqn._dqn), step, self.dqn._dqn)
        else:
            Pyllab.scores_and_indices_readapter_float(self.b2, self.b1, len(l), Pyllab.get_array_size_scores_dueling_categorical_dqn(dqn._dqn), step, self.dqn._dqn)
        for i in range(len(l)):
            dqn = l[i]
            Pyllab.memcopy_vector_to_scores_dueling_categorical_dqn(dqn._dqn, self.b1[i])
            Pyllab.memcopy_vector_to_indices_dueling_categorical_dqn2(dqn._dqn, self.b2[i])
'''      
cdef class Iq:
    cdef Pyllab.iq* _q
    cdef Model network
    ...
    cdef int* _actions
    cdef int* done
    cdef int training_dataset_size
    cdef int input_size
    cdef int batch_size
    cdef int threads
    cdef int feed_forward_flag
    ...
    def __cinit__(self, Model network, states, actions, done, int batch_size = 128,int threads = 8, int feed_forward_flag = PY_FULLY_FEED_FORWARD, int training_mode = PY_GRADIENT_DESCENT,
                  int adaptive_clipping_flag = 0, int gd_flag = PY_ADAM, int lr_decay_flag = 0, int lr_epoch_threshold = 10, float momentum = 0.9, float alpha1 = 0.01, float alpha2 = 0.5,
                  float gamma = 0.99, float beta1 = PY_BETA1_ADAM, float beta2 = PY_BETA2_ADAM, float beta3 = PY_BETA3_ADAMOD, float k_percentage = 1, float adaptive_clipping_gradient_value = 0.01,
                  float lr = 0.001, float lr_maximum = 0.1, float lr_minimum = 0.0001, float lr_decay = 0.0001):
    
    iq* init_iq(model* q_network, model** q_networks, float** states, int* actions, int* done, uint size, uint state_size, uint batch_size, uint threads,
