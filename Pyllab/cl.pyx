...
def copy_vector_to_params_cl(cl c, vector):
    ...
    if c._does_have_arrays and c._does_have_learning_parameters:
        Pyllab.memcopy_vector_to_params_cl(c._cl,<float*>v.data.as_floats)

def copy_params_to_vector_cl(cl c, vector):
    v = np.arange(c.get_array_size_params(),dtype=np.dtype("f"))
    if c._does_have_arrays and c._does_have_learning_parameters:
        Pyllab.memcopy_params_to_vector_cl(c._cl,<float*>v.data.as_floats)
        return v
    return None
...
def copy_vector_to_weights_cl(cl c, vector):
    ...
    cdef array.array v = vector_is_valid(vector)
    if c._does_have_arrays and c._does_have_learning_parameters:
        Pyllab.memcopy_vector_to_weights_cl(c._cl,<float*>v.data.as_floats)

def copy_weights_to_vector_cl(cl c, vector):
    v = np.arange(c.get_array_size_weights(),dtype=np.dtype("f"))
    if c._does_have_arrays and c._does_have_learning_parameters:
        Pyllab.memcopy_weights_to_vector_cl(c._cl,<float*>v.data.as_floats)
        return v
    return None
...
def copy_vector_to_scores_cl(cl c, vector):
    ...
    cdef array.array v = vector_is_valid(vector)
    if c._does_have_arrays and c._does_have_learning_parameters:
        Pyllab.memcopy_vector_to_scores_cl(c._cl,<float*>v.data.as_floats)

def copy_scores_to_vector_cl(cl c, vector):
    v = np.arange(c.get_array_size_scores(),dtype=np.dtype("f"))
    if c._does_have_arrays and c._does_have_learning_parameters:
        Pyllab.memcopy_scores_to_vector_cl(c._cl,<float*>v.data.as_floats)
        return v
    return None
...
def copy_vector_to_derivative_params_cl(cl c, vector):
    ...
    if c._does_have_arrays and c._does_have_learning_parameters and not c._is_only_for_feedforward:
        Pyllab.memcopy_vector_to_derivative_params_cl(c._cl,<float*>v.data.as_floats)

def copy_derivative_params_to_vector_cl(cl c, vector):
    v = np.arange(c.get_array_size_params(),dtype=np.dtype("f"))
    if c._does_have_arrays and c._does_have_learning_parameters and not c._is_only_for_feedforward:
        Pyllab.memcopy_derivative_params_to_vector_cl(c._cl,<float*>v.data.as_floats)
        return v
    return None
...
def compare_score_cl_with_vector(cl c1, vector, cl c_output):
    ...
    if c1._does_have_arrays and c1._does_have_learning_parameters and c_output._does_have_arrays and c_output._does_have_learning_parameters:
        Pyllab.compare_score_cl_with_vector(c1._cl, <float*>v.data.as_floats,c_output._cl)
