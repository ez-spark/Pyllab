...
def copy_vector_to_params_fcl(fcl f, vector):
    ...
    if f._does_have_arrays and f._does_have_learning_parameters:
        Pyllab.memcopy_vector_to_params(f._fcl,<float*>v.data.as_floats)

def copy_params_to_vector_fcl(fcl f):
    v = np.arange(f.get_array_size_params(),dtype=np.dtype("f")
    cdef array.array v = vector_is_valid(vector)
    if f._does_have_arrays and f._does_have_learning_parameters:
        Pyllab.memcopy_params_to_vector(f._fcl,<float*>v.data.as_floats)
        return v
    return None
...
def copy_vector_to_weights_fcl(fcl f, vector):
    ...
    cdef array.array v = vector_is_valid(vector)
    if f._does_have_arrays and f._does_have_learning_parameters:
        Pyllab.memcopy_vector_to_weights(f._fcl,<float*>v.data.as_floats)

def copy_weights_to_vector_fcl(fcl f, vector):
    v = np.arange(f.get_array_size_weights(),dtype=np.dtype("f")
    if f._does_have_arrays and f._does_have_learning_parameters:
        Pyllab.memcopy_weights_to_vector(f._fcl,<float*>v)
        return v
    return None
...
def copy_vector_to_scores_fcl(fcl f, vector):
    ...
    cdef array.array v = vector_is_valid(vector)
    if f._does_have_arrays and f._does_have_learning_parameters:
        Pyllab.memcopy_vector_to_scores(f._fcl,<float*>v.data.as_floats)

def copy_scores_to_vector_fcl(fcl f, vector):
    v = np.arange(f.get_array_size_scores(),dtype=np.dtype("f")
    if f._does_have_arrays and f._does_have_learning_parameters:
        Pyllab.memcopy_scores_to_vector(f._fcl,<float*>v.data.as_floats)
        return v
    return None
...
def copy_vector_to_derivative_params_fcl(fcl f, vector):
    ...
    if f._does_have_arrays and f._does_have_learning_parameters and not f._is_only_for_feedforward:
        Pyllab.memcopy_vector_to_derivative_params(f._fcl,<float*>v.data.as_floats)

def copy_derivative_params_to_vector_fcl(fcl f, vector):
    v = np.arange(f.get_array_size_params(),dtype=np.dtype("f")
    if f._does_have_arrays and f._does_have_learning_parameters and not f._is_only_for_feedforward:
        Pyllab.memcopy_derivative_params_to_vector(f._fcl,<float*>v.data.as_floats)
        return v
    return None
...
def compare_score_fcl_with_vector(fcl f1, vector, fcl f_output):
    ...
    if f1._does_have_arrays and f1._does_have_learning_parameters and f_output._does_have_arrays and f_output._does_have_learning_parameters:
        Pyllab.compare_score_fcl_with_vector(f1._fcl, <float*>v.data.as_floats,f_output._fcl)
